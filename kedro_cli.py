...
def build_docs(open_docs):
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=shutdown_candidate_predictor"]
    )
    ...
    call(
        [
            ...
            "-o",
            "docs/source",
            "src/shutdown_candidate_predictor",
        ]
    )
...
def _build_jupyter_command(
    ...
    if not all_kernels:
        project_name = "Shutdown Candidate Predictor"
...
def convert_notebook(all_flag, overwrite_flag, filepath):
    ...
    *Note*: Make sure your notebooks have unique names!
    ...
    kedro_package_name = "shutdown_candidate_predictor"
